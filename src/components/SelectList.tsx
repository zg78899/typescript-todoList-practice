...
import { setSelectedList } from '../store/action/list.action';
        ...
        <div className="control has-icons-left">
          <div className="select fullwidth">
            <select className="fullwidth" onChange={selectChangeHandler} >
              ...
              {Object.keys(lists).length > 0 && Object.values(lists).map((list)=>(<option key={list.id} value={list.id}>{list.name}</option>)) }
            </select>
          </div>
